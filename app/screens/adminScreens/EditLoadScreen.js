...
const EditLoadScreen = (props) => {
  const theme = useTheme();
  const load = props.route.params.load ?? {};
  ...
  let initialSite = {
    ...
    terminalRates: {
      rateToronto:
        load?.terminal == "Toronto" ? parseInt(load?.terminalRate) : 0,
      rateHamilton:
        load?.terminal == "Hamilton" ? parseInt(load?.terminalRate) : 0,
      rateOakville:
        load?.terminal == "Oakville" ? parseInt(load?.terminalRate) : 0,
      rateNanticoke:
        load?.terminal == "Nanticoke" ? parseInt(load?.terminalRate) : 0,
    },
  };
  ...
  const [rate, setRate] = useState(load?.terminalRate);
  const [splits, setSplits] = React.useState(load?.splits);
  const [selectedIndex, setSelectedIndex] = React.useState(
    terminalMap.map((e) => e.name).indexOf(load?.terminal)
  );
  const [terminal, setTerminal] = React.useState(load?.terminal);
  const [date, setDate] = React.useState(new Date(load?.date));
  const [uptLink, setUptLink] = React.useState(load?.uptLink);
  const [waiting, setWaiting] = React.useState(load?.waitingTime);
  ...
  const SubmitForm = async () => {
    ...
    const result = await loadsApi.updateLoad(load?._id, data);
    if (!result.ok) {
            ...
            >
              ...
              <Layout level="2" style={styles.outsideContainer}>
                <View>
                  ...
                  <Input
                    returnKeyType="next"
                    onChangeText={setUptLink}
                    defaultValue={load?.uptLink}
                  />
                </View>
              </Layout>
              <Layout level="2" style={styles.outsideContainer}>
              ...
              <Layout level="2" style={styles.outsideContainer}>
                <View style={[styles.innerContainer, { marginTop: 10 }]}>
                  <View style={{ flex: 1, width: "49%", margin: "1%" }}>
                    <Button onPress={SubmitForm}>UPDATE</Button>
                  </View>
                  <View style={{ flex: 1, width: "49%", margin: "1%" }}>
                    <Button
                      appearance="outline"
