...
const AllFiles_Admin = (prop) => {
  ...
  const refreshData = async () => {
    const response = await shiftDocsApi.getShiftDocs({ folder: folder?._id });
  ...
  const handleDelete = async (file) => {
    //setLoading(true);
    const result = await shiftDocsApi.deleteFile(file._id);
    //setLoading(false);
    if (!result.ok) {
      return Alert.alert(result.data.error);
    }
    ...
    setFilteredData(
      filteredData.filter(function (f) {
  ...
  const renderListItem = ({ item, index }) => (
    ...
    >
      ...
      >
        <AppText category="s1">{item?.fileName}</AppText>
        <Button
          status="danger"
          appearance="outline"
          size="tiny"
        ...
        <Layout level="2" style={{ flex: 1 }}>
          <SafeAreaView>
            <TopNavigation
              {...prop}
              alignment="start"
              title={`All Files in ${folder?.name} `}
              accessoryRight={() => (
                <TopNavigationAction
                  icon={PlusIcon}
                  onPress={() => {
